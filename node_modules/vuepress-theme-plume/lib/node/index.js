...
var perf = {};
function perfMark(mark) {
  perf[mark] = performance.now();
}
function perfLog(mark, isDebug = false) {
...
async function findConfigPath(app, configPath) {
  ...
  if (configPath) {
    ...
    if (existsSync(path9) && (await fsp2.stat(path9)).isFile()) {
      return path9;
    }
  }
  extensions.forEach(
...
function mergeLocaleOptions(target, source) {
  const res = {};
...
import { getFullLocaleConfig as getFullLocaleConfig2, isPlainObject as isPlainObject2 } from "@vuepress/helper";
function resolveProvideData(app, plugins) {
  return {
    // 注入水印配置
    __PLUME_WM_FP__: isPlainObject2(plugins.watermark) ? plugins.watermark.fullPage !== false : true,
    // 注入多语言配置
...
import { entries as entries3, isBoolean, isPlainObject as isPlainObject3 } from "@vuepress/helper";
...
function resolveThemeData(app, options) {
  ...
  options.contributors = isPlainObject3(options.contributors) ? { mode: options.contributors.mode || "inline" } : isBoolean(options.contributors) ? options.contributors : true;
  options.changelog = !!options.changelog;
  if (isPlainObject3(themeData.bulletin)) {
    const { enablePage: _, ...opt } = themeData.bulletin;
    themeData.bulletin = opt;
  }
  if (isPlainObject3(themeData.blog)) {
  ...
  entries3(options.locales || {}).forEach(([locale, opt]) => {
    ...
    if (isPlainObject3(themeData.locales[locale].bulletin)) {
...
function resolveOptions(localeOptions, options) {
  ...
  return {
    ...
    exclude: uniq3([".vuepress/**/*", "node_modules", ...options?.exclude ?? []]),
    frontmatter: configs
  };
}
...
import { ensureLeadingSlash as ensureLeadingSlash3, isPlainObject as isPlainObject4 } from "@vuepress/helper";
var uuid = 1e4;
var cache = {};
...
function autoCategory(page, options) {
  ...
  const blog = isPlainObject4(options.blog) ? options.blog : {};
...
import { isFunction, isPlainObject as isPlainObject5 } from "@vuepress/helper";
function enableBulletin(page, options) {
  if (isPlainObject5(options.bulletin)) {
...
import { isPlainObject as isPlainObject6 } from "@vuepress/helper";
...
function getPlugins({
  ...
  if (pluginOptions.git ?? isProd) {
    ...
    const changelogOptions = isPlainObject6(options.changelog) ? options.changelog : {};
    plugins.push(gitPlugin({
      createdTime: false,
      updatedTime: options.lastUpdated !== false,
      contributors: isPlainObject6(options.contributors) || options.contributors === true ? {
        avatar: true,
  ...
  if (pluginOptions.markdownInclude !== false) {
    plugins.push(markdownIncludePlugin(isPlainObject6(pluginOptions.markdownInclude) ? pluginOptions.markdownInclude : {}));
  }
  if (pluginOptions.watermark) {
    plugins.push(watermarkPlugin({
      delay: 300,
      enabled: true,
      ...isPlainObject6(pluginOptions.watermark) ? pluginOptions.watermark : {}
    }));
  }
  ...
  if (pluginOptions.sitemap !== false && isProd) {
    const sitemapOptions = isPlainObject6(pluginOptions.sitemap) ? pluginOptions.sitemap : {};
  ...
  if (pluginOptions.seo !== false && hostname && isProd) {
    const seoOptions = isPlainObject6(pluginOptions.seo) ? pluginOptions.seo : {};
...
import { isPlainObject as isPlainObject7 } from "vuepress/shared";
var PRESET = [
...
async function prepareArticleTagColors(app, localeOptions) {
  ...
  const blog = isPlainObject7(localeOptions.blog) ? localeOptions.blog : {};
...
import { isArray as isArray2, uniq as uniq4 } from "@pengzhanbo/utils";
import { entries as entries4, isLinkAbsolute as isLinkAbsolute2, isLinkHttp, isPlainObject as isPlainObject8 } from "@vuepress/helper";
...
function getSidebarData(app, locales) {
  ...
  entries5(locales).forEach(([localePath, sidebar]) => {
    ...
    } else if (isPlainObject9(sidebar)) {
...
var bulletinFiles = {};
process4.on("exit", () => bulletinFileWatcher?.close());
async function prepareThemeData(app, pluginOptions) {
  ...
  return (app) => {
    ...
    return {
      ...
      onInitialized: async (app2) => {
        await createPages(app2);
      },
      onPrepared: async (app2) => {
        await prepareThemeData(app2, pluginOptions);
