...
import { isPlainObject as isPlainObject3 } from "vuepress/shared";
...
function createCopyCodeButtonRender(app, options) {
  ...
  getLocalePaths(app).forEach((path4) => {
    locales[path4] = userLocales[path4] || copyCodeButtonLocales[path4 === "/" ? root : path4];
  });
...
import { colors, logger as logger3 } from "vuepress/utils";
...
var vueRE = /-vue$/;
function getInlineTransformers({ attrs, lang, enabledTwoslash, whitespace, twoslash }) {
  const vPre2 = vueRE.test(lang) ? "" : "v-pre";
  const inlineTransformers = [
    transformerCompactLineOptions(attrsToLines(attrs))
  ];
  if (enabledTwoslash) {
    const { compilerOptions, ...twoslashOptions } = isPlainObject2(twoslash) ? twoslash : {};
  ...
  return (str, language, attrs = "") => {
    ...
    try {
      ...
      logger3.error(
        e?.message,
        "\n",
        e?.stack ? colors.gray(String(e?.stack)) : ""
      );
      return str;
    }
  };
...
// src/node/highlight/resolveTsPaths.ts
import fs from "node:fs/promises";
import path from "node:path";
import process3 from "node:process";
async function resolveTsPaths() {
  const tsconfigPath = path.join(process3.cwd(), "tsconfig.json");
  try {
    const tsconfig = JSON.parse(await fs.readFile(tsconfigPath, "utf-8"));
    const paths = tsconfig.compilerOptions?.paths ?? void 0;
    const baseUrl = tsconfig.compilerOptions?.baseUrl;
    if (baseUrl && paths) {
      const dirname = path.join(process3.cwd(), baseUrl);
...
async function scanLanguages(app) {
  ...
  for (const file of files) {
    const filepath = path2.join(source, file);
...
import { getDirname, path as path3 } from "vuepress/utils";
...
function shikiPlugin({
  ...
  const copyCodeOptions = isPlainObject3(copyCode) ? copyCode : {};
  return {
    ...
    extendsMarkdown: async (md, app) => {
      ...
      if (!options.languages || !options.languages.length) {
        ...
        if (options.languages.length) {
          logger.warn(`You have not configured \`${colors2.cyan("plugins.shiki.languages")}\`. It has been detected that you are using \`${colors2.green(JSON.stringify(options.languages))}\`. Please add it to the \`${colors2.cyan("plugins.shiki.languages")}\` configuration.`);
        }
        if (app.env.isDebug) {
      ...
      if (options.twoslash) {
        ...
        if (paths) {
          options.twoslash = isPlainObject3(options.twoslash) ? options.twoslash : {};
    ...
    extendsMarkdownOptions: (options2) => {
      if (options2.vPre !== false) {
        const vPre2 = isPlainObject3(options2.vPre) ? options2.vPre : { block: true };
        if (vPre2.block) {
          options2.vPre ??= {};
