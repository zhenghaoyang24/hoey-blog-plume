...
**CSS动画是**CSS3的新特性，想用动画做一个简单的页面加载动画。

<!-- more -->


首先建一个盒子用于装四个小正方形，并用 Grid 布局让正方形2x2排列。因为要让四个正方形绕中心旋转，则给父元素加上旋转动画，为了让旋转之间有停顿，因此旋转从20再开始。  
为了让四个正方形从中间分散，直接改变父元素 `grid-gap` 则可实现。最后给正方形在加上 `border-radius` 看起来就更加和谐了。
