...
permalink: /qa/basic/javascript/bhm4jxoq/
outline: 2
---

## 变量声明

<Question :questions="['变量声明的方式与它们的区别？']" />

---

在 JavaScript 中，有多种变量声明方式，主要包括使用 `var`、`let`、`const` 关键字，其中 `let`、`const`
是在 ES6 中引入的。
...
若仍然想要使用 var 声明且依次打印 i，可以使用 IIFE 立即执行函数的特性：==立即执行函数会形成一个独立的作用域。==
...
const code2 = `function testVarAndLet(){
  for (var i = 0; i < 5; ++i) {
    setTimeout(() => console.log(i), 0);
  }
  for (let i = 0; i < 5; ++i) {
