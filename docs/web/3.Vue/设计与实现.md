...
Proxy 只能包装对象（引用类型），不能直接包装基本类型（string、number、boolean、null、undefined、symbol、bigint）。
因为基本类型是不可变的、按值传递的，没有属性、没有方法（除了临时装箱）。Proxy 需要拦截属性操作，而基本类型本身没有属性。
要想监听基本类型，需要将其包装成对象。

资料参考：[深入响应式系统](https://cn.vuejs.org/guide/extras/reactivity-in-depth.html)。